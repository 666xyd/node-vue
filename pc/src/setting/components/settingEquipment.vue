<template>
    <div class="setting-page">
        <div class="form">
            <AddButton title="新增装备分类" @add="add"></AddButton>
        </div>
        ...
        <equipment-classify v-model="popupShow" v-if="popupShow" @success="success"
                      :is-edit="isEdit" :edit-item="editItem"></equipment-classify>
    </div>
</template>

<script>
import AddButton from '../../components/AddButton';
import equipmentClassify from "@/equipment/components/equipmentClassify";
...
export default {
    ...
    methods: {
        ...
        //删除
        toDelete(row){
            this.$confirm(`是否删除装备分类--${row.name}?`, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async () => {
                await this.$http.delete(`rest/equipmentClassify/${row._id}`)
                this.$message({
                    type: 'success',
                    message: '删除成功!'
                });
