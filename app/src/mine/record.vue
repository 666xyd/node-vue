<template>
    <div class="mine-like">
        <p class="fc-primary fs-xl fw-700 flex al-center title">
            <van-icon name="like-o" size="16" style="margin-right: 0.308rem"/>我浏览过</p>
        ...
        <div v-else>
            <van-empty description="您还没有看过文章呢" />
    ...
    methods: {
        ...
        getArticleList(){
            for(let i = 0; i < this.list.length; i++){
                this.$http.get(`rest/articleInfo/articleId/${this.list[i].id}`).then((res)=>{
                    if(res.data){
